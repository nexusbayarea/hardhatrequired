...
function rowToCandidate(row: Record<string, unknown>): GeoCandidate {
  return {
    id: row.id as string,
    canonicalKey: row.canonical_key as string || row.company_name as string,
    ...
    latitude: (row.latitude as number) ?? (row.lat as number | undefined),
    longitude: (row.longitude as number) ?? (row.lng as number | undefined),
    ...
    intelligenceScore: (row.intelligence_score as number) || 0,
    evaluatedRing: (row.evaluated_ring as number) || 0,
  };
}

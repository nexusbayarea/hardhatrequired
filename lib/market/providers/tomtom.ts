...
export class TomTomProvider implements DiscoveryProvider {
  ...
  async search(params: DiscoveryParams): Promise<Partial<Company>[]> {
    ...
    const query = encodeURIComponent(
      params.searchQueries?.[0] || params.verticalConfig?.industryName || 'business'
    );
    ...
    try {
      ...
      return data.results.map((poi: any) => ({
        id: `tt_${poi.id}`,
        companyName: poi.poi?.name || 'Unknown Business',
      ...
      console.warn('[TomTomProvider] POI search failed:', error);
      return [];
    }
  }
}
